...
const char * const PIVX_CONF_FILENAME = "mirai.conf";
const char * const PIVX_PID_FILENAME = "mirai.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Mirainodes only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\mirai
// Windows >= Vista: C:\Users\Username\AppData\Roaming\mirai
// Mac: ~/Library/Application Support/mirai
// Unix: ~/.mirai
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mirainodes";
#else
    fs::path pathRet;
    ...
    return pathRet / "Mirainodes";
#else
    // Unix
    return pathRet / ".mirai";
#endif
#endif
}
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\miraiParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\miraiParams
    // Mac: ~/Library/Application Support/miraiParams
    // Unix: ~/.mirai-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MirainodesParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "MirainodesParams";
#else
    // Unix
    return pathRet / ".mirai-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty mirai.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Mirainodes config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override mirai.conf
