...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Mirainodes client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Mirainodes"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Mirainodes"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
