    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of MIRAI earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 MIRAI");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("MIRAI")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair MIRAI, zMIRAI
