...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    if (nHeight > 700000)	return 7500 * COIN;
    if (nHeight > 600000)	return 5000 * COIN;
    if (nHeight > 400000)	return 4000 * COIN;
...
{
    if (nHeight > 900000)	return 7 * COIN;
    if (nHeight > 700000)	return 6 * COIN;
    if (nHeight > 600000)	return 5   * COIN;
    if (nHeight > 400000)	return 4   * COIN;
    if (nHeight > 200000)	return 3.5   * COIN;
    if (nHeight > 100000)	return 3   * COIN;
    if (nHeight > 50000)	return 2.5   * COIN;
    if (nHeight > 1000)		return 2   * COIN;
    if (nHeight >= 1)		return 150 * COIN;
    return 0 * COIN;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 MIRAI tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 MIRAI tx -> 1 confirmation
