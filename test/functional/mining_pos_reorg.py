...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected MIRAI and zMIRAI supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zMIRAIsupply']:
                assert_equal(node['zMIRAIsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check MIRAI and zMIRAI supply at the beginning
        # ------------------------------------------
        # zMIRAI supply: 2 coins for each denomination
        ...
        # MIRAI supply: block rewards minus burned fees for minting
        ...
        # Verify that MIRAI and zMIRAI supplies were properly updated after the spends and reorgs
        self.log.info("Check MIRAI and zMIRAI supply...")
